...
const TimeLine =() => {
    ...
    const {isLoading, data, error} = useWikiData({pageName: 'History_of_Mesopotamia'});
    ...
    <Divider width='50vw' zIndex='-11' />
      ...
      {error && <Code fontSize='15px' marginBottom='50px'>{error}</Code>}
      {isLoading ? <Code fontSize='15px' marginBottom='50px'>Building Timeline...</Code> :
                 ...
                 <TimelineSeparator > 
                      ...
                      <TimelineContent style={{marginBottom:'5vh'}}>
                        <TimelineBox data={c}>
                          ...
                          <Box role="group"  
                            style={{pointerEvents:deviceType == 'touchOnly' && 'none'}} >
                              
                              <Box 
                              padding='10px' 
                                _groupHover={{
                                  backgroundColor: colorMode === 'dark' ? '#1c2541' : '#EEEEEE',
